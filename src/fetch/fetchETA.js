...
export async function fetchBulkCTBETA(route){
    ...
    try{
        const companyCode = route.company.includes('CTB') ? 'CTB' :
                            route.company.includes('NWFB') ? 'NWFB' : null;
        let etaReq = [];
        if (route.stops[0].altId){
            etaReq = route.stops.map( stop => axios(`https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/eta/${companyCode}/${stop.altId}/${route.routeNo}`));
        } else {
            etaReq = route.stops.map( stop => axios(`https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/eta/${companyCode}/${stop.stopId}/${route.routeNo}`));
        }
